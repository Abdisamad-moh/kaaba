...
class AllJobs
{
    ...
    public function __construct(
        private readonly EmployerJobsRepository $jobs, 
    ...
    public function getJobs(): array
    {
        ...
        $jobs = $this->jobs->getJobPaginator(0, $this->page * ($this->home ? self::PER_PAGE_HOME : self::PER_PAGE), false, $this->job_title, $this->job_category_id, $this->country_id)
                ->getQuery()->getResult();
        if($this->home == false):
            $check_for_more = $this->jobs->getJobPaginator(0, ($this->page + 1) * self::PER_PAGE, true, $this->job_title, $this->job_category_id, $this->country_id)->getQuery()->getSingleScalarResult();
            if($check_for_more == count($jobs)) $this->has_more = false;
        endif;
